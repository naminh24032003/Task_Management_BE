...
// NewDatabaseAndClient creates a new MongoDB database connection and also returns the client
// The client is needed for UnitOfWork transactions (MongoDB sessions require the client)
func NewDatabaseAndClient(cfg *Config) (*mongo.Database, *mongo.Client, func(), error) {
	ctx, cancel := context.WithTimeout(context.Background(), cfg.ConnectTimeout)
	defer cancel()
	...
	fmt.Println("✅ Connected to MongoDB successfully (with client for transactions)")
